...
public class TuneCurrentLimitCommand extends Command {
  ...
  private double getmoduleStatorCurrent(int module) {
    return driveSubsystem.getModule(module).getDriveMotor().getStatorCurrent().getValue();
  }

  private double getModuleSupplyCurrent(int module) {
    return driveSubsystem.getModule(module).getDriveMotor().getSupplyCurrent().getValue();
  }
}
