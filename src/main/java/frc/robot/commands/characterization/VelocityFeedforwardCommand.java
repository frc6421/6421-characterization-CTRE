...
public class VelocityFeedforwardCommand extends Command {
  ...
  public void setSetRobotVelocity(double velocity) {
    this.setRobotVelocity = MathUtil.clamp(velocity, -1 * DriveConstants.SPEED_AT_12_VOLTS_METERS_PER_SEC,
  ...
  private double getModuleVoltage(int module) {
    return driveSubsystem.getModule(module).getDriveMotor().getMotorVoltage().getValue();
  }

  // TODO verify module units: rpm or m/s?
  private double getModuleVelocity(int module) {
    return driveSubsystem.getModule(module).getDriveMotor().getVelocity().getValue();
  }
  ...
  private double getRobotVelocity() {
    return driveSubsystem.getSwerveDriveKinematics()
